...
def main(calculator: BaseAggregationCalculator | None = None) -> None:
    ...
    display_case_header("PŘÍPAD ROZPOČET", opinions, metadata)
    ...
    print(f"\nOdhad nejlepšího kompromisu: {best_compromise_centroid:.2f} miliard Kč (těžiště)")
    print(
        f"Fuzzy číslo: ({best_compromise.lower_bound:.2f}, "
        f"{best_compromise.peak:.2f}, {best_compromise.upper_bound:.2f})"
    )
    print(
        f"Rozsah: [{best_compromise.lower_bound:.2f}, {best_compromise.upper_bound:.2f}] miliard Kč"
    )
    ...
    print(
        f"\nVýsledek naznačuje, že přibližně {best_compromise_centroid:.2f} miliard Kč"
