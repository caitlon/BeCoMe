...
def _make_scaled_experts(count: int) -> list[ExpertOpinion]:
    """Build a list of experts with linearly scaled fuzzy numbers.

    :param count: Number of experts to create (max 6)
    :return: List of ExpertOpinion instances
    """
    data = [
        ("E1", 1.0, 2.0, 3.0),
        ("E2", 3.0, 5.0, 7.0),
    ...
    return [
        ExpertOpinion(
            expert_id=eid,
            opinion=FuzzyTriangleNumber(lower_bound=a, peak=c, upper_bound=b),
        )
        for eid, a, c, b in data[:count]
    ]
