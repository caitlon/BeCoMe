...
def test_redis_valid_after_key_has_a_ttl():
    client = fakeredis.FakeStrictRedis()
    RedisRevocationStore(client).set_user_valid_after(uuid4(), datetime.now(UTC))
    keys = client.keys("user:valid_after:*")
    assert keys
    # The cutoff must expire eventually rather than lingering forever per user.
