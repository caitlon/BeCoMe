...
When you remove or refactor functionality, delete it completely. Git history is the record of
what used to exist, so leftovers do not belong in the working tree. Flag any of these:
...
A refactor is incomplete until you update every test and call site that referenced the old code.
...
attribute or as an explicit parameter. That keeps each formula verifiable against the documented
BeCoMe method. A bare literal in the middle of a calculation hides intent and blocks
parametrization.
...
name describes the behavior it verifies. Each test keeps a single logical focus instead of
